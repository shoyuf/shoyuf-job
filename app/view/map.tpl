      ...
      res.data.list.forEach(el => {
        ...
        if (!isNaN(longitude) && !isNaN(latitude)) {
          ...
          const inner = `<div class="info-top">
              <div>${el.companyShortName}-${el.companyFullName}<br/>${el.positionName}-${el.salary}</div><img src="https://webapi.amap.com/images/close2.gif" onclick="closeInfoWindow()">
            </div>
            <div class="info-middle" style="background-color: white;">
              ...
              工资构成：${el.companyLabelList.toString()}<br/>
              学历要求: ${el.education}<br/>
              发布时间: ${el.createTime}<br/>
              公司优势: ${el.positionAdvantage}<br/>
              年资: ${el.workYear}<br/>
              <a href="${el.url}" target="_blank">详细信息-${el.jobForm}</a>
            </div>`
